...
public class BasicJMM_06_Causality {
    ...
    */
    ...
    @Outcome(id = "1, 0", expect = ACCEPTABLE_INTERESTING, desc = "Seeing $y, but not $x!")
    @State
    public static class ReleaseNoAcquire {
        static final VarHandle VH;

        static {
            try {
                VH = MethodHandles.lookup().findVarHandle(ReleaseNoAcquire.class, "y", int.class);
            } catch (NoSuchFieldException | IllegalAccessException e) {
                throw new IllegalStateException(e);
            }
        }
    ...
    @Outcome(id = "1, 0", expect = FORBIDDEN, desc = "Seeing $y, but not $x!")
    @State
    public static class StoreStoreLoadLoad {
        int x;
        int y;
    ...
    @Outcome(id = "1, 0", expect = FORBIDDEN, desc = "Seeing $y, but not $x!")
    @State
    public static class StoreStoreWithoutLoadLoad {
        static final VarHandle VX, VY;

        static {
            try {
                VX = MethodHandles.lookup().findVarHandle(StoreStoreWithoutLoadLoad.class, "x", int.class);
                VY = MethodHandles.lookup().findVarHandle(StoreStoreWithoutLoadLoad.class, "y", int.class);
            } catch (NoSuchFieldException | IllegalAccessException e) {
                throw new IllegalStateException(e);
            }
        }
        ...
        public void actor2(II_Result r) {
            r.r1 = y;
            UNSAFE.storeFence(); // loadstore + storestore
//            UNSAFE.loadFence(); // loadstore + loadload
//            UNSAFE.fullFence(); // loadstore + storestore + loadload + storeload
//            setZ();
            r.r2 = x;
        }
