 ...
 *
 * <p>A {@link Set} projection of a MyConcurrentHashMap may be created
 * (using {@link #newKeySet()} or {@link #newKeySet(int)}), or viewed
 * (using {@link #keySet(Object)} when only keys are of interest, and the
 * mapped values are (perhaps transiently) not used or all take the
 * same mapping value.
 *
 * <p>A MyConcurrentHashMap can be used as a scalable frequency map (a
 * form of histogram or multiset) by using {@link
 * java.util.concurrent.atomic.LongAdder} values and initializing via
 * {@link #computeIfAbsent computeIfAbsent}. For example, to add a count
 * to a {@code MyConcurrentHashMap<String,LongAdder> freqs}, you can use
 ...
 *
 * <p>MyConcurrentHashMaps support a set of sequential and parallel bulk
 ...
 * MyConcurrentHashMap are not ordered in any particular way, and may be
 ...
 * from those of MyConcurrentHashMap: Any non-null result returned
...
public class MyConcurrentHashMap<K, V> extends AbstractMap<K, V>
    ...
    static final <K, V> Node<K, V> tabAt(Node<K, V>[] tab, int i) {
        return (Node<K, V>) TAB.getAcquire(tab, i);
    }

    static final <K, V> boolean casTabAt(Node<K, V>[] tab, int i,
                                         Node<K, V> c, Node<K, V> v) {
        return TAB.compareAndSet(tab, i, c, v);
    }

    static final <K, V> void setTabAt(Node<K, V>[] tab, int i, Node<K, V> v) {
        TAB.setRelease(tab, i, v);
    }
    ...
    static final class TreeBin<K, V> extends Node<K, V> {
        ...
        static {
            try {
                ...
                WAITERTHREAD = U.objectFieldOffset(TreeBin.class.getDeclaredField("waiter"));
            } catch (Exception e) {
                throw new Error(e);
            }
        }
    ...
    static {
        try {
            SIZECTL = U.objectFieldOffset(MyConcurrentHashMap.class.getDeclaredField("sizeCtl"));
            TRANSFERINDEX = U.objectFieldOffset(MyConcurrentHashMap.class.getDeclaredField("transferIndex"));
            BASECOUNT = U.objectFieldOffset(MyConcurrentHashMap.class.getDeclaredField("baseCount"));
            CELLSBUSY = U.objectFieldOffset(MyConcurrentHashMap.class.getDeclaredField("cellsBusy"));
    ...
    private static final VarHandle TAB = MethodHandles.arrayElementVarHandle(Node[].class);
